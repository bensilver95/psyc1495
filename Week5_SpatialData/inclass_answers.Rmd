...
```{r}
plot_usmap(regions = 'counties',include = c('NY','NJ'))
```
...
```{r}
plot_usmap(regions = 'counties',include = c('NY','NJ'), data = countypov, values = "pct_pov_2021")
```

Let's use FIPS codes to Zoom in on NYC. You can figure out what the FIPS codes are by Googling.

```{r}
plot_usmap(regions = 'counties',include = c(36061,36047,36081,36005,36085), data = countypov, values = "pct_pov_2021")
```
...
plot_usmap(regions = 'counties',
           include = c(36061,36047,36081,36005,36085), 
           data = countypov, 
           values = "pct_pov_2021") +
  scale_fill_continuous(low = 'black',high = 'red') + # ggplot syntax
  theme(legend.position = 'right') # ggplot syntax
```
...
```{r}
ggplot() +
  geom_sf(data = nyc_counties_pov,
          aes(fill = pct_pov_2021)) +
  theme_void()
```
...
```{r}
schools <- read_csv('2019_schools.csv')
schools
```
...
ggplot() +
  geom_sf(data = nyc_counties_pov,aes(fill = pct_pov_2021)) +
  geom_sf(data = nyc_trees, color = 'lightgreen') +
  theme_void()
```
