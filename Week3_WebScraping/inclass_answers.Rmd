...
```{r}
popNY_demo_labels <- get_estimates(
  year = 2023,
  geography = "county",
  product = "characteristics",
  state = 'New York',
  breakdown = c('RACE','SEX'),
  breakdown_labels = T
)
```
...
n <- n %>% 
  mutate(freq_rescaled = Frequency*10000000000)
...
```{r}
n2 <- ngram(
  c('mass incarceration','war on drugs'),
  year_start = 1900, 
  year_end = 2022)

# let's again multiply
n2 <- n2 %>% 
  mutate(freq_rescaled = Frequency*10000000000)
```
...
ggram(c("mass incarceration","war on drugs"), 
      year_start = 1900,
      year_end = 2022)
```
