---
title: 'In class tutorial: Data cleaning (with answers)'
...
```{r}
library(tidyverse)
suny <- read_csv('SUNY_enrollment.csv')
```
...
```{r}
suny_undergrad <- suny %>% 
  select(-c(Graduate_FullTime,Graduate_PartTime))
```
...
```{r}
suny_undergrad_long <- suny_undergrad %>% 
  pivot_longer(cols = c(Undergraduate_FullTime,Undergraduate_PartTime),
...
```{r}
ggplot(suny_undergrad_long %>% 
         filter(Institution_Type == 'Community Colleges'),
       aes(x = Student_type, y = Enrollment)) +
...
```{r}
applications <- ngram(
  c('college application','job application'),
  year_start = 1922, 
  year_end = 2022)

applications <- applications %>% 
  mutate(Frequency = Frequency * 1000000000)
```
...
```{r}
applications_wider <- applications %>% 
...
```{r}
summary(lm(Frequency ~ Phrase, data = applications))
```
...
```{r}
applications <- applications %>% 
  mutate(Phrase.e = if_else(Phrase == 'college application', -0.5, 0.5))
```
...
```{r}
summary(lm(Frequency ~ Phrase.e, data = applications))
```
...
```{r}
summary(lm(Frequency ~ Phrase.e * Year, data = applications))
```
...
```{r}
applications <- applications %>% 
  mutate(Year.c = as.vector(scale(Year, center = T, scale = F)))
```

Now, let's run our regression again, but this time with `Year.c`:

```{r}
summary(lm(Frequency ~ Phrase.e * Year.c, data = applications))
```
...
```{r}
ggplot(applications %>% 
         mutate(Phrase.e = as.factor(Phrase.e)), 
       aes(x = Year, y = Frequency, color = Phrase.e)) +
  geom_jitter(alpha = .4) +
