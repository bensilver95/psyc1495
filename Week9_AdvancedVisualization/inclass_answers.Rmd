...
```{r}
mayor <- mayor %>% 
  mutate(Total = rowSums(select(.,-c(AssemblyDistrict,Borough)),
                         na.rm = T))
```
...
```{r}
mayor <- mayor %>% 
  mutate(Mamdani_total = `Zohran Kwame Mamdani (Democratic)` + 
           `Zohran Kwame Mamdani (Working Families)`,
    Mamdani_pct = Mamdani_total/Total * 100,
    Cuomo_pct = `Andrew M. Cuomo (Fight and Deliver)`/Total * 100)
```
...
```{r}
mayor_select <- mayor %>% 
  select(AssemblyDistrict,Borough,Mamdani_pct, Cuomo_pct)
```

Now, we can pivot!

```{r}
mayor_select_long <- mayor_select %>% 
  pivot_longer(cols = c(Mamdani_pct,Cuomo_pct),
               names_to = 'Candidate',
               values_to = 'Vote_pct')
```
...
```{r}
ggplot(mayor_select_long,
       aes(x = Candidate, y = Vote_pct)) +
...
```{r}
mayor_select_long <- mayor_select_long %>% 
  mutate(Candidate.e = if_else(Candidate == 'Mamdani_pct',1,0))
```

Now, we'll run our model.

```{r}
summary(lm(Vote_pct ~ Candidate.e, data = mayor_select_long))
...
ggplot(mayor_select_long,
       aes(x = Candidate, y = Vote_pct)) +
  geom_bar(stat = "summary", fun = "mean", fill = 'palegreen3') +
...
```{r}
mayor_select_summary <- mayor_select_long %>% 
  group_by(Candidate) %>% 
  summarize(Mean_pct = mean(Vote_pct),
            SE_pct = sd(Vote_pct)/sqrt(n()))
```
...
```{r}
ggplot(mayor_select_summary,
       aes(x = Candidate, y = Mean_pct)) +
...
ggplot(mayor_select_summary,
  ...
  geom_errorbar(aes(ymin = Mean_pct - SE_pct,
                ...
                width = .2) +
  lims(y = c(10,84)) +
  geom_text(aes(label = Mean_pct), vjust = 2.5)
```
...
ggplot(mayor_select_summary,
  ...
  geom_errorbar(aes(ymin = Mean_pct - SE_pct,
                ...
                width = .2) +
  lims(y = c(10,84)) +
  geom_text(aes(label = round(Mean_pct,2)), vjust = 2.5)
```
...
ggplot(mayor_select_summary,
  ...
  lims(y = c(10,84)) +
  geom_text(aes(label = round(Mean_pct,2)), vjust = 2.5) +
  geom_text(x = 2, y = 60, label = 'Mamdani surpassed 50%')
```
